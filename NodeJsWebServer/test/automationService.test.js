...
test('chart reading requests validate ranges and leave the raw readings API unchanged', async () => {
  reading('20');
  const res = response();
  await controller.listReadings({ params: { id: 1 }, query: { view: 'chart', key: 'temperature' } }, res);
  assert.equal(res.statusCode, 200);
